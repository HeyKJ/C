...
14는 가장 작은 최소값을 가지고 있으므로 자식이 아예 없거나 오른쪽 자식이 하나 있을 수 있다.(최소값은 가장 왼편에 위치하기 때문에 최소값이 왼쪽 자식을 가질 수 없다)  
자식이 없다면 문제가 14를 10에 위치시키면 되지만 만일 14가 15라는 오른쪽 자식을 갖고 있었다면 15와 17을 연결해주고
14는 10의 자리로 위치하면 된다.  
