더블 링크드 리스트
*단방향으로만 탐색이 가능하던 링크드 리스트와는 달리 양방향 탐색이 가능
...
새로운 노드가 추가될 때
*추가 전
	(헤드)[NULL][데이터][노드1] <-> (노드1)[헤드][데이터][테일] <-> (노드2 추가)[노드1][데이터][테일] <-> (테일)[노드1][데이터][NULL]
*노드2가 추가되었기 때문에 노드1의 다음 노드는 테일 <-> 노드2로 변경되고 
테일의 이전 노드는 노드1 <-> 노드2로 변경
*추가 후
	(헤드)[NULL][데이터][노드1] <-> (노드1)[헤드][데이터][노드2] <-> (노드2)[노드1][데이터][테일] <-> (테일)[노드2][데이터][NULL]

노드가 삭제될 때
*삭제 전
	(헤드)[NULL][데이터][노드1] <-> (노드1)[헤드][데이터][노드2] <-> (노드2 삭제)[노드1][데이터][테일] <-> (테일)[노드2][데이터][NULL]
*노드2가 삭제되었기 때문에 노드1의 다음 노드는 노드2 -> 테일로 변경되고
테일의 이전 노드는 노드2 -> 노드1으로 변경
*삭제 후
	(헤드)[NULL][데이터][노드1] <-> (노드1)[헤드][데이터][테일] <-> (테일)[노드1][데이터][NULL]
